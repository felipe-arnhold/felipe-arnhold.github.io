...
first.p_s[i]=s}}}}
function relink(s,dt){var s2=s.ts_next
s.time+=dt
if(s.dur)
s.dur-=dt
s.seqst=1
...
s.ts_next.seqst=1
s2=s
if(dt<0){do{s2=s2.prev}while(s2&&!s2.dur)
if(s2){s2.dur+=dt
s2.pdur=s2.dur/play_fac
s2=s2.ts_next
if(s2==s)
s2=null}}else{if(!s.ts_next){s2=null}else{s2=s2.ts_next
while(s2&&!s2.seqst)
s2=s2.ts_next}}
...
return
s.midgen=1
var g,i,n,t,d,prev,next=s.next
n=0
...
s2=s2.ts_next
if(s2.time>=tim&&s2.ts_next){s.ts_prev=s2.ts_prev
s.ts_prev.ts_next=s
s.ts_next=s2
s2.ts_prev=s}else{s.ts_prev=s2
s2.ts_next=s}}
chnm=abc2svg.chnm
